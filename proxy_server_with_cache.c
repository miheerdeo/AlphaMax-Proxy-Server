...
struct cache_element{
    char* data;         //data stores HTML response is acts as a value against the key url.
    int len;          //length of data 
    char* url;        //In cache it we store (key,value) pair so key is the url and value is the HTML response.
	time_t lru_time_track;    //Time based LRU cache implementation.So used for time tracking.
    cache_element* next;    //Cache implemented as a linked list.
};
...
int proxy_socketId;					// socket descriptor of proxy server(Single socket to intract with the proxy)
...
{
	// Creating Socket from our code(client) to connect to the remote server
	...
	// Gethostname has a functionality for resilving the domain specifient in the http request in the ip address needed for the coonection
    ...
    printf("The domain %s belongs to IP address: %s\n", host_addr, ip_address);

	//Now we need our client socket(remoteSocket) which our code to connect to the server so for that to give parameters to the connect function we need to provide the address in sockaddr format for that we need to put this ip address in the structure.
...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	...
	int remoteSocketID = connectRemoteServer(request->host, server_port); // the function returns a the socketID of socket connected to the server.

	if(remoteSocketID < 0)
		return -1;
    
	//Sending the contructed HTTP request to the remote socket descriptor for the server.
	int bytes_send = send(remoteSocketID, buf, strlen(buf), 0);
    printf("Sent HTTP request to server. Bytes sent: %d\n", bytes_sent);
	bzero(buf, MAX_BYTES);
    //Recieve the first few bytes of the repsonse;
	bytes_send = recv(remoteSocketID, buf, MAX_BYTES-1, 0);
    //Creating a temprorray buffer for storing in the cache.
	...
	printf("HTTP request sucessfully handled.\n");
	free(temp_buffer);
...
void* thread_fn(void* socketNew)
{
	...
	bzero(buffer, MAX_BYTES);								// Making buffer zero.Alternate use memcpy() easier it is
	bytes_send_client = recv(socket, buffer, MAX_BYTES, 0); // Receiving the Request of client by proxy server
	
	while(bytes_send_client > 0)
	{
		len = strlen(buffer);
        // "\r\n\r\n" is the end of the HTTP request.
		if(strstr(buffer, "\r\n\r\n") == NULL)
		{	
	...
	if( temp != NULL){
		...
		printf("Cache Hit!!!. Request is present in the proxy server cache. \n\n");
		printf("%s\n\n",response);
